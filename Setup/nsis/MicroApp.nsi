...
Section "MicroApp (required)" SecApp
  ...
  ; facades the .NET Framework build binds against (resource deserialization, Span<T>)
  ...
  File "${SRC}\onnxruntime.dll"
  File "${SRC}\onnxruntime_providers_shared.dll"
  File "${SRC}\msvcp140.dll"
  File "${SRC}\msvcp140_1.dll"
  File "${SRC}\vcruntime140.dll"
  File "${SRC}\vcruntime140_1.dll"
  ; Remove Background's IS-Net model
  SetOutPath "$INSTDIR\Models"
  File "${SRC}\Models\isnet-general-use.onnx"
  SetOutPath "$INSTDIR"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\Models\isnet-general-use.onnx"
